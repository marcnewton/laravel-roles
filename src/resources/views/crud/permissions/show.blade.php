                ...
                <div class="{{ $containerClass }} {{ $bootstrapCardClasses }}">
                    <div class="{{ $containerHeaderClass }}">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span id="card_title">
                                @isset($typeDeleted)
                                    {!! trans('laravel-roles::admin.titles.show-permission-deleted', ['name' => $item->name]) !!}
                                @else
                                    {!! trans('laravel-roles::admin.titles.show-permission', ['name' => $item['permission']->name]) !!}
                                @endisset
                            </span>
                            <div class="pull-right">
                                @isset($typeDeleted)
                                    <a href="{{ route('laravel-roles::permissions-deleted') }}" class="btn btn-outline-danger btn-sm float-right" data-toggle="tooltip" data-placement="left" title="{{ trans('laravel-roles::admin.tooltips.back-permissions-deleted') }}">
                                        <i class="fa fa-fw fa-reply-all" aria-hidden="true"></i>
                                        {!! trans('laravel-roles::admin.buttons.back-to-permissions-deleted') !!}
                                    </a>
                                @else
                                    <a href="{{ route('laravel-roles::roles.index') }}" class="btn btn-outline-secondary btn-sm float-right" data-toggle="tooltip" data-placement="left" title="{{ trans('laravel-roles::admin.tooltips.back-permissions') }}">
                                        <i class="fa fa-fw fa-reply-all" aria-hidden="true"></i>
                                        {!! trans('laravel-roles::admin.buttons.back-to-permissions') !!}
                                    </a>
                                @endisset
                            </div>
                        </div>
                    ...
                    <div class="{{ $containerBodyClass }}">
                        <ul class="list-group">
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravel-roles::admin.cards.permission-info-card.permission-id') !!}
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravel-roles::admin.cards.permission-info-card.permission-name') !!}
                                <span class="badge badge-pill">
                                    @isset($typeDeleted)
                                        {{ $item->name }}
                                    @else
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravel-roles::admin.cards.permission-info-card.permission-slug') !!}
                                <span class="badge badge-pill">
                                    @isset($typeDeleted)
                                        {{ $item->slug }}
                                    @else
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravel-roles::admin.cards.permission-info-card.permission-model') !!}
                                <span class="badge badge-pill">
                                    @isset($typeDeleted)
                                        {{ $item->model }}
                                    @else
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravel-roles::admin.cards.permission-info-card.permission-desc') !!}
                                <span class="badge badge-pill">
                                    @isset($typeDeleted)
                                        @if($item->desc)
                                            {{ $item->desc }}
                                        @else
                                            <span class="text-muted">
                                                {!! trans('laravel-roles::admin.cards.permission-info-card.none') !!}
                                            </span>
                                        @endif
                                    @else
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravel-roles::admin.cards.permission-info-card.permission-roles') !!}
                                @isset($typeDeleted)
                            ...
                            <li id="accordion_roles_users" class="list-group-item accordion @if($item['users']->count() > 0) list-group-item-action accordion-item collapsed @endif" data-toggle="collapse" href="#collapse_roles_users">
                                <div class="d-flex justify-content-between align-items-center" @if($item['users']->count() > 0) data-toggle="tooltip" title="{{ trans("laravel-roles::tooltips.show-hide") }}" @endif>
                                    {!! trans('laravel-roles::admin.cards.permission-info-card.permission-users') !!}
                                ...
                                @if($item['users']->count() > 0)
                                    <div id="collapse_roles_users" class="collapse" data-parent="#accordion_roles_users" >
                                        <table class="table table-striped table-sm mt-3">
                                            <caption>
                                                @isset($typeDeleted)
                                                    {!! trans('laravel-roles::admin.cards.permissions-card.permissions-table-users-caption', ['permission' => $item->name]) !!}
                                                @else
                                                    {!! trans('laravel-roles::admin.cards.permissions-card.permissions-table-users-caption', ['permission' => $item['permission']->name]) !!}
                                                @endisset
                                            </caption>
                                            <thead>
                                                <tr>
                        ...
                        <div class="row">
                            <div class="col-sm-6 mt-3">
                                @isset($typeDeleted)
                                    @include('laravel-roles::forms.restore-item', ['style' => 'large', 'type' => 'permission', 'item' => $item])
                                @else
                                    <a class="btn btn-sm btn-secondary btn-block text-white mb-0" href="{{ route('laravel-roles::permissions.edit', $item['permission']->id) }}" data-toggle="tooltip" title="{{ trans("laravel-roles::tooltips.edit-permission") }}">
                            ...
                            <div class="col-sm-6 mt-3">
                                @isset($typeDeleted)
                                    @include('laravel-roles::forms.destroy-sm', ['large' => 'large', 'type' => 'Permission' ,'item' => $item])
                                @else
                                     @include('laravel-roles::forms.delete-sm', ['type' => 'Permission' ,'item' => $item['permission'], 'large' => true])
                                @endisset
                            </div>
                        </div>
                    </div>
...
@section(config('roles.bladePlacementJs'))
    ...
    @endif
    @include('laravel-roles::scripts.confirm-modal', ['formTrigger' => '#confirmRestore'])
