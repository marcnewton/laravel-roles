    ...
    <div class="card">
        <div class="card-header bg-default">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span id="card_title">
                    {!! trans('laravel-roles::admin.titles.roles-card') !!}
                </span>
...
@endphp
                        <li id="accordion_roles_{{ $itemKey }}" class="list-group-item accordion @if($item['users']->count() > 0 || $item['permissions']->count() > 0) list-group-item-action accordion-item collapsed @endif" data-toggle="collapse" href="#collapse_roles_{{ $itemKey }}">
                            <div class="d-flex justify-content-between align-items-center" @if($item['users']->count() > 0 || $item['permissions']->count() > 0) data-toggle="tooltip" title="{{ trans("laravel-roles::tooltips.show-hide") }}" @endif>
                                <span class="badge badge-light role-name">
                                    {!! trans('laravel-roles::admin.titles.role-card') !!} <strong class="{{ $indRoleTextClass }}">{{ $item['role']->name }}</strong>
                                </span>
                                <div class="text-right">
                                    <span class="badge badge-dark">
                                        <small>
                                            {{ trans('laravel-roles::admin.cards.level', ['level' => $item['role']->level]) }}
                                        </small>
                                    </span>
                                ...
                                <div id="collapse_roles_{{ $itemKey }}" class="collapse" data-parent="#accordion_roles_{{ $itemKey }}" >
                                    ...
                                    @if($item['users']->count() > 0)
                                        <table class="table table-striped table-sm mt-3">
                                            <caption>
                                                {!! trans('laravel-roles::admin.cards.role-card.table-users-caption', ['role' => $item['role']->name]) !!}
                                            </caption>
                                            <thead>
                                                <tr>
                                    ...
                                    @if($item['permissions']->count() > 0)
                                        <table class="table table-striped table-sm mt-3">
                                            <caption>
                                                {!! trans('laravel-roles::admin.cards.role-card.table-permissions-caption', ['role' => $item['role']->name]) !!}
                                            </caption>
                                            <thead>
                                                <tr>
